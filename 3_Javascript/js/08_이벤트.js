...
function check1(btn){
    ...
    // 현재 버튼 요소의 배경색을 얻어와 저장
    const bgColor = btn.style.backgroundColor;

    // 버튼이 클릭 될 때마다 pink <-> yello 변경
...
// *** 표준 이벤트 모델 작성법 ***
// 요소.addEventListener("이벤트정류", 이벤트핸들러(함수));
...
let inputColor = document.querySelector("#input3");
let boxBg = document.querySelector("#box3");

inputColor.addEventListener("keyup", function(e){

    if(e.key == "Enter"){ // 엔터가 눌러지고 떼어졌을 때
        boxBg.style.backgroundColor=inputColor.value;
    }

});

boxBg.addEventListener("click", function(e){
    // e.target : 이벤트가 발생한 요소 == this 로 써도 됨
    alert(e.target.style.backgroundColor);

    //alert(boxBg.style.backgroundColor); // 또는
    //alert(this.style.backgroundColor); // 로 해도 됨
    inputColor.value ="";
    inputColor.focus();
