...
//const test; // 상수는 선언과 동시에 초기화 반드시 진행.

// PI = 1.23;
// Uncaught TypeError: Assignment to constant variable.
// 상수로 배정되어 있다.
// -> 상수에 새로운 값을 대입할 수 없는데, 재대입을 하고있어 오류사항이다.
...
//console.log(num2); // 중간에 에러 발생하면 다음줄로 못내려감 // Uncaught ReferenceError: num1 is not defined
...
console.log(test2);
// -> test2도 접근 가능.
// -> 왜? 함수레벨 스코프인 var 는 일반블록은 무시하기 때문

// 함수안에서는 어떻게 될까?
function example() { // 이렇게 함수 형태로 만들어진
    // 코드 블록은 함수레벨 scope라고 함.
...
// -> 호이스팅 되어 var a; 라는 선언부는 최상단으로 올라감
// -> 선언은 이미 되어있으나 a에 값이 대입되진 않았기 때문에
// undefined 를 반환하는 것.
...
// 초기화 코드는 원래 위치에서 이루어짐

console.log(a);

// 2. let / const 의 호이스팅
// : let/const 로 선언된 변수는 선언만 해당 스코프의 최상단으로 호이스팅 됨
// var와 달리 let/const 로 선언된 변수는 
...
// -> x의 선언이 호이스팅 됨.
// 그러나 TDZ 에 놓여 초기화 전까지 접근불가
// -> 초기화가 이루어지기 전까지 해당 변수는 TDZ 놓이며,
// 이 기간동안 변수에 접근하려고 하면
...
console.log("numbers 배열 값 중 0 번째 : ", numbers[0], typeof numbers[0]);
console.log("numbers 배열 값 중 1 번째 : ", numbers[1], typeof numbers[1]);
console.log("numbers 배열 값 중 2 번째 : ", numbers[2], typeof numbers[2]);
console.log("numbers 배열 값 중 3 번째 : ", numbers[3], typeof numbers[3]);
